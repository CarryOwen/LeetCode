		...
		for (int sublen = 1; sublen <= len; sublen++)
		{
			...
			{	
				for (int k= 0; k < sublen; k++)
				{
					...
					tmpvector.push_back(nums[j+k]);
				}
				if (sublen==tmpvector.size())
				{
					int tmpSum=0;
					...
					maxSums=maxSums>tmpSum?maxSums:tmpSum;
					tmpvector.clear();
				}

			}
		}
		cout<<"maxSums: "<<maxSums<<endl;
		return maxSums;
    }
};
int main()
